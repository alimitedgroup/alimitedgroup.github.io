...
Data l'assenza di un componente eventuali correzioni alle decisioni ri-
portate in questo documento saranno debitamente verbalizzate a seguito di
nuova riunione.
...
Durante la riunione è stata presentata la seguente immagine:

#align(center, image("../../assets/altd.png", height: 7cm))

Si è proposto di utilizzarla come logo per rappresentare il nostro gruppo.
...
- *Telegram*: è stato proposto di utilizzare un gruppo Telegram per comunicare
  di informazioni generali riguardante le attività del gruppo (ad esempio, per
  proporre nuove riunioni)
- *Discord*: è stato proposto di utilizzare un server Discord per
...
È stato inoltre proposto di contattare il proponente M31 per porre alcune
ulteriori domande riguardanti il capitolato, tra cui:

- Richiesta maggiori dettagli in merito alla gestione della concorrenza
- Richiesta maggiori dettagli in merito allo svolgimento degli Unit Test
